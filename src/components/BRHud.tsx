...
interface BRHudProps {
  ...
  posX: number;
  posZ: number;
  otherPlayers: { uid: string; x: number; z: number; alive: boolean }[];
  onBack: () => void;
}

export default function BRHud({ hp, maxHp, armor, weapon, kills, aliveCount, totalPlayers, zoneRadius, killFeed, posX, posZ, otherPlayers, onBack }: BRHudProps) {
    ...
    <div className="absolute inset-0 pointer-events-none">
      ...
      <div className="absolute top-0 inset-x-0 p-2 flex items-center justify-between pointer-events-auto">
        ...
        <div className="flex items-center gap-3 bg-black/60 px-3 py-1.5 rounded-full border border-white/10">
          <span className="text-[10px] font-mono text-slate-400">
            Alive: <span className="text-emerald-400 font-bold">{aliveCount}</span>
      ...
      <div className="absolute bottom-0 right-0 m-3 w-28 h-28 bg-black/80 rounded-lg border border-white/10 overflow-hidden pointer-events-auto">
        <div className="relative w-full h-full" style={{ transform: 'scaleX(-1)' }}>
          ...
          {/* Players */}
          {otherPlayers.map((p) => (
            <div key={p.uid} className={`absolute w-1.5 h-1.5 rounded-full ${p.alive ? 'bg-red-400' : 'bg-slate-700'}`} style={{
      ...
      {/* Kill feed (top-right) */}
      <div className="absolute top-12 right-2 space-y-0.5 pointer-events-auto w-44">
